...
export function classifyRemoteType(text: string): RemoteType {
  ...
  if (
    matchesRegex(normalized, HYBRID_REGEX) ||
    matchesKeywords(normalized, REMOTE_KEYWORDS.HYBRID)
  )
    return RemoteType.HYBRID;
  if (
    matchesRegex(normalized, REMOTE_REGEX) ||
    matchesKeywords(normalized, REMOTE_KEYWORDS.REMOTE)
  )
